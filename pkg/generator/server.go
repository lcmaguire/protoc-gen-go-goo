...
func (g *Generator) generateConnectServer(gen *protogen.Plugin, file FileInfo, services []string) {
	fileName := strings.ToLower("cmd" + "/" + file.GoPackageName + "/" + "main.go")
	f := gen.NewGeneratedFile(fileName, protogen.GoImportPath("."))
	...
	f.P(templates.ExecuteTemplate(connectgo.ServerTemplate, serverData{Services: resgisteredServices, ConnectGenImportPath: connectGenImportPath, ServiceImports: servicePaths}))
}
